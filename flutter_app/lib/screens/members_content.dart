...
class _MembersContentState extends State<MembersContent>
                              ...
                              child: Stack(
                                children: [
                                  DataTable2(
                                    ...
                                    rows: filteredMembers.asMap().entries.map((
                                      entry,
                                    ) {
                                      final idx = entry.key;
                                      ...
                                      final baseRowColor = idx.isEven
                                          ? colorScheme.surface
                                          : zebraColor;
                                      final emailValue = (member.email ?? '')
                                          .trim();
                                      final phoneValue = (member.phone ?? '')
                                          .trim();
                                      final addressValue =
  ...
  Widget _buildPhotoCell(Member member) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: (member.isActive ? Colors.green : Colors.grey).withValues(
              alpha: 0.25,
            ),
            blurRadius: 8,
  ...
  Future<void> _deleteSelectedMembers() async {
    ...
    final confirmed = await showPremiumConfirm(
      ...
      message:
          'Delete ${ids.length} selected member(s)? This action cannot be undone.',
      confirmLabel: 'Delete',
